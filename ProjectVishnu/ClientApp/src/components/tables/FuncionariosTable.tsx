...
export default function FuncionariosTable({
  ...
  selectable,
  funcionariosIdList,
  funcionariosIdListSetter,
}: Props) {
  const navigate = useNavigate();
  useEffect(() => {
    if (selectable && !(funcionariosIdList && funcionariosIdListSetter)) {
      throw new Error(
        "If you mark the table as selectable, you must set funcionariosIdList and funcionariosIdListSetter"
      );
    }
  }, []);
  const addToFuncionariosList = (id: number) => {
    funcionariosIdListSetter((funcionariosIdList: number[]) => [
      ...funcionariosIdList!,
      id,
    ]);
  };
  const removeFuncionarioFromList = (id: number) => {
    funcionariosIdListSetter((funcionariosIdList: number[]) => [
      ...funcionariosIdList.filter((funcId) => funcId != id),
    ]);
  };
        ...
        <Table className="table table-striped" aria-labelledby="tabelLabel">
          ...
          <Tbody>
            {funcionarios.map((funcionario) => (
              <Tr className="data-table-row" key={funcionario.nif}>
                {selectable && (
                  <Td className="!border-r-slate-100">
                    <Checkbox
                      ...
                      onChange={(e) =>
                        e.target.checked
                          ? addToFuncionariosList(funcionario.id)
                          : removeFuncionarioFromList(funcionario.id)
                      }
                    />
                  </Td>
                )}
                <Td onClick={() => redirectToFuncionario(funcionario.id)}>
                  {funcionario.nome}
                </Td>
                <Td onClick={() => redirectToFuncionario(funcionario.id)}>
                  {funcionario.nif}
